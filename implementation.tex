...
As you can see there are only two interchangeable modules. Those are the Hour Registration and the Shift market. But this does not mean that the other domains should not be modules. If every domain is a module it makes it easy for us to change a basic function if an enterprise wants that.
...
We will test all of these frameworks against the same use case:
...
The assumption is made that the database exists where \texttt{root} is the username and password and the web framework used is the name of the database table.
